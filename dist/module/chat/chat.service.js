...
class ChatService {
    async createConversation(userId, title, mode, documentId, documentName) {
    ...
    async sendMessage(userId, conversationId, message, mode, file) {
        ...
        if (file && mode === 'AGENTIC') {
            ...
            aiResponse = await pythonBackendService.agentUploadAndChat(file.buffer, file.fileName, message);
            ...
            aiResponse = await pythonBackendService.ragChat(message, conversation.documentId, conversationHistory);
        }
